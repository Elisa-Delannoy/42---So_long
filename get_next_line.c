...
char	*ft_temp(char **newbuffer, char *buffer, int i)
{
	...
	if (i < 0)
	{
		temp = ft_strjoin_line(*newbuffer, buffer);
		if (temp == NULL)
			return (NULL);
		if (*newbuffer != NULL)
			free (*newbuffer);
	...
	if (i >= 0)
	{
		temp = ft_strdup_line(*newbuffer + i + 1);
		if (temp == NULL)
			return (NULL);
		if (*newbuffer != NULL)
			free (*newbuffer);
...
char	*ft_read(int fd, char *buffer, char **newbuffer)
{
	...
	m = 1;
	while (m != 0 && ft_strchr_line(*newbuffer, '\n') == NULL)
	{
...
char	*get_next_line(int fd)
{
	...
	if (newbuffer == NULL)
		return (NULL);
	content = ft_calloc_line((ft_strlen_line(newbuffer) + 1), sizeof(char));
	if (content == NULL)
